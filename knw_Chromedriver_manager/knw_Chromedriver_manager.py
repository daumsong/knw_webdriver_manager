...
def get_chrome_version():
    ...
    if platform.system() == 'Windows':
        try:
            ...
            print(f"크롬브라우저 버전 정보 획득 실패: {str(e)}")
        else:
            return version
    ...
    for path in paths:
        ...
        try:
            ...
            print(f"{path} 이 경로에서 크롬브라우저 버전 정보 획득 실패: {str(e)}")
        else:
            break
...
def download(version):
    ...
    try: #폴더만들기
        ...
        print("웹드라이버 다운로드 폴더생성 실패")

    try: #크롬드라이버 목록 받아오기
        response = requests.get('https://googlechromelabs.github.io/chrome-for-testing/latest-patch-versions-per-build-with-downloads.json') # https://github.com/GoogleChromeLabs/chrome-for-testing#json-api-endpoints
